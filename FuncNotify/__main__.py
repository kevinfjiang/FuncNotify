...
def main(): 
    ...
    class _ParseKwargs(argparse.Action):
        ...
        Wrapped to nobody can access this as it's exclusive to parsing
        """    
        translation_dict = {"true" : True,
                            "True" : True,
                            "false": False,
                            "False": False}
        class _CollectionParse:
            CloseList = {'[': ']',
                         '{': '}',
                         '(': ')',
                         '"': '"',
        ...
        def add_kwarg(self, namespace, key, value):
            """Adds args to args.kwargs so it can be used as kwargsed"""            
        ...
        def add_collection_parse(self, namespace, CollectParseObj, value):
            ...
            if CollectParseObj.build(value):
                self.add_kwarg(namespace, CollectParseObj.key, CollectParseObj.__eval__())
        
        def __call__(self, parser, namespace, values, option_string=None):
            setattr(namespace, self.dest, dict())
            setattr(namespace, "_unrecognized_args", list())
            CollectParseObj=_ParseKwargs._CollectionParse("")
            ...
            for value in values:
                if not CollectParseObj and re.match('^[a-zA-Z0-9_]+(=[^\n\t\r]+)$', value):
                    ...
                    if value[0] in ["[", "{", "("]:
                        ...
                        CollectParseObj=_ParseKwargs._CollectionParse(key)
                        self.add_collection_parse(namespace, CollectParseObj, value)
                    else:
                        value = _ParseKwargs.translation_dict.get(value, value) # Ensures bools are properly parsed
    ...
    parser.add_argument('-k', '--kwargs', nargs='*', action=_ParseKwargs,
